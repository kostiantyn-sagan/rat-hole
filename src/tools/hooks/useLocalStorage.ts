...
export const useLocalStorage = <_, TValue>(
    key: string,
    innitialValue: TValue,
    ...
    const [ storedValue, setStoredValue ] = useState(() => {
        try {
            ...
            console.log(
                `local storage error by key: ${APP_NAME}:${key}. Npm package store error.`,
            );
    ...
    const setValue = (value: TValue) => {
        try {
            ...
            console.log(
                `local storage error by key: ${APP_NAME}:${key}. Dont forget about KEY and VALUE arguments.`,
            );
        }
    };
